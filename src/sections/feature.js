...
export default function Contact() {
  ...
  return (
    <section id="contact" style={{ padding: '96px 0', background: '#07070f', position: 'relative', overflow: 'hidden' }}>

      {/* Aurora */}
      <div style={{ position: 'absolute', top: 0, right: '10%', width: 500, height: 400, background: 'radial-gradient(circle, rgba(141,68,139,0.25) 0%, transparent 65%)', filter: 'blur(80px)', pointerEvents: 'none' }} />
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px', position: 'relative' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 72, alignItems: 'start' }} className="contact-grid">
          ...
          {/* Left */}
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 100, border: '1px solid rgba(192,132,252,0.3)', background: 'rgba(141,68,139,0.12)', marginBottom: 24 }}>
              <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#C084FC' }}>Get In Touch</span>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              {INFO.map((info, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: 'rgba(141,68,139,0.2)', color: '#C084FC' }}>{info.icon}</div>
                  <div>
            ...
            <div style={{ display: 'flex', gap: 24, marginTop: 48, paddingTop: 48, borderTop: '1px solid rgba(255,255,255,0.08)' }}>
              {['Fast Response', 'NDA Ready', 'Agile Teams'].map(badge => (
                <div key={badge} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <svg width={14} height={14} fill="none" viewBox="0 0 24 24" stroke="#C084FC" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg>
          ...
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 28, padding: '40px', backdropFilter: 'blur(20px)' }}>
            {status === 'sent' ? (
              <div style={{ textAlign: 'center', padding: '48px 0' }}>
                <div style={{ width: 64, height: 64, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #8D448B, #6366f1)', margin: '0 auto 20px', boxShadow: '0 0 32px rgba(141,68,139,0.4)' }}>
                ...
                <button onClick={() => setStatus(null)} style={{ fontSize: 13, color: '#C084FC', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
                  Send another →
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit}>
                  ...
                  {[
                    ...
                    <div key={field.name}>
                      ...
                      <input type={field.type} name={field.name} value={form[field.name]} onChange={handleChange} required placeholder={field.placeholder} style={inputStyle}
                        onFocus={e => e.target.style.borderColor = '#8D448B'}
                ...
                <button type="submit" disabled={status === 'sending'} style={{ width: '100%', padding: '14px', borderRadius: 12, border: 'none', cursor: 'pointer', fontSize: 15, fontWeight: 700, color: '#fff', background: 'linear-gradient(135deg, #8D448B 0%, #6366f1 100%)', transition: 'all 0.2s', opacity: status === 'sending' ? 0.7 : 1, fontFamily: 'inherit', boxShadow: '0 4px 24px rgba(141,68,139,0.3)' }}
